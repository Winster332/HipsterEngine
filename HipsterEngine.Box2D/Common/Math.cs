...
namespace Box2DX.Common
{
  public class Math
  {
    public static readonly float FLOAT32_EPSILON = 1.192093E-07f;
    public static readonly float FLOAT32_MAX = float.MaxValue;
    public static readonly ushort USHRT_MAX = ushort.MaxValue;
    public static readonly byte UCHAR_MAX = byte.MaxValue;
    public static readonly int RAND_LIMIT = (int) short.MaxValue;
    ...
    {
      Math.Convert convert = new Math.Convert();
      convert.x = x;
      float num = 0.5f * x;
    ...
    {
      return (float) (2.0 * (double) ((float) (Math.s_rnd.Next() & Math.RAND_LIMIT) / (float) Math.RAND_LIMIT) - 1.0);
    }

    public static float Random(float lo, float hi)
    {
      float num = (float) (Math.s_rnd.Next() & Math.RAND_LIMIT) / (float) Math.RAND_LIMIT;
      return (hi - lo) * num + lo;
    }
    ...
    {
      Vec2 vec2 = new Vec2();
      vec2.Set(Math.Abs(a.X), Math.Abs(a.Y));
      return vec2;
    }
    ...
    public static Mat22 Abs(Mat22 A)
    {
      ...
      mat22.Set(Math.Abs(A.Col1), Math.Abs(A.Col2));
      return mat22;
    }
    ...
    {
      Vec2 vec2 = new Vec2();
      vec2.Set((float) ((double) A.Col1.X * (double) v.X + (double) A.Col2.X * (double) v.Y), (float) ((double) A.Col1.Y * (double) v.X + (double) A.Col2.Y * (double) v.Y));
      return vec2;
    }
    ...
    {
      Vec2 vec2 = new Vec2();
      vec2.Set(Vec2.Dot(v, A.Col1), Vec2.Dot(v, A.Col2));
      return vec2;
    }
    ...
    {
      Mat22 mat22 = new Mat22();
      mat22.Set(Math.Mul(A, B.Col1), Math.Mul(A, B.Col2));
      return mat22;
    }
